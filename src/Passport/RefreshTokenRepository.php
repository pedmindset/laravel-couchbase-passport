...
class RefreshTokenRepository implements RefreshTokenRepositoryInterface
{
    ...
    public function persistNewRefreshToken(RefreshTokenEntityInterface $refreshTokenEntity)
    {
        $this->database->table('oauth_refresh_tokens')->insert([
            'oauth_refresh_token_id' => $id = $refreshTokenEntity->getIdentifier(),
    ...
    public function revokeRefreshToken($tokenId)
    {
        $this->database->table('oauth_refresh_tokens')
                    ->where('oauth_refresh_token_id', $tokenId)->update(['revoked' => true]);
    }
    ...
    public function isRefreshTokenRevoked($tokenId)
    {
        $refreshToken = $this->database->table('oauth_refresh_tokens')
                    ->where('oauth_refresh_token_id', $tokenId)->first();
